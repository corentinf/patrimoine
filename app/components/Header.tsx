...
function ProfileMenu() {
  ...
  return (
    <div ref={ref} className="relative">
      ...
      {open && (
        <div className="absolute right-0 top-full mt-2 w-52 bg-white border border-sand-200 rounded-xl shadow-lg py-1.5 z-50">
          {email && (
            <p className="px-3 py-1.5 text-xs text-ink-400 border-b border-sand-100 mb-1 truncate">{email}</p>
          )}
          <div className="px-3 py-1 border-b border-sand-100 mb-1">
            <p className="text-[10px] font-semibold text-ink-400 uppercase tracking-wider">Connections</p>
          </div>
